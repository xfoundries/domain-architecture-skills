...
- In infrastructure modules, group adapters by technical shape before external-system name when that improves clarity, such as `persistence`, `readmodel`, `client.<external-system>`, `messaging`, `cache`, and `file`. Keep HTTP/API `*Request` and `*Response` DTOs in primary adapter packages; use application-owned `*Command` and `*Result` models for primary-port boundaries when that naming fits the project.
