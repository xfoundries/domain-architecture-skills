...
For Spring Boot applications, jfoundry Spring auto-configuration injects `DomainEventContext` into `AbstractPersistenceRepository` internally. Business repository constructors should not expose `DomainEventContext`. For non-Spring or manual assembly, call `setDomainEventContext(...)` after constructing the repository.
